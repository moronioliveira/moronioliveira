# Olá, sejam bem vindos ao meu Github! 👋
### Fullstack Software Engineer | Java (Spring Boot) & JavaScript (React, TypeScript) Specialist

---

## 👨‍💻 Sobre Mim
Engenheiro de Software Fullstack focado no ecossistema **Java (Spring Boot)** e **JavaScript/TypeScript (React)**. Experiência na construção de **RESTful APIs** robustas, escaláveis e seguras com **Spring Security (JWT)**, arquitetura de microsserviços e **BFF (Backend For Frontend)**, além de interfaces reativas e modernas no front-end. Domínio em modelagem de dados SQL/NoSQL e forte compromisso com **Clean Code** e arquitetura em camadas.

---

## 🕹️ Meu Ecossistema de Contribuições
Aqui você pode vê o rastro dos meus commits sendo "devorados" pelo Pacman em tempo real:

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/moronioliveira/moronioliveira/output/pacman-contribution-graph-dark.svg">
  <img alt="pacman contribution graph" src="https://raw.githubusercontent.com/moronioliveira/moronioliveira/output/pacman-contribution-graph.svg">
</picture>

---

## 🛠️ Tecnologias & Hard Skills

| Categoria | Tecnologias |
| :--- | :--- |
| **Back-end** | ![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white) ![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=spring-boot&logoColor=white) ![Spring Security](https://img.shields.io/badge/Spring_Security-6DB33F?style=flat-square&logo=spring-security&logoColor=white) ![Hibernate](https://img.shields.io/badge/Hibernate-59666C?style=flat-square&logo=Hibernate&logoColor=white) ![Swagger](https://img.shields.io/badge/Swagger-85EA2D?style=flat-square&logo=swagger&logoColor=black) |
| **Front-end** | ![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB) ![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black) ![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white) ![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=black) |
| **Bancos de Dados** | ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white) ![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white) |
| **DevOps & Ferramentas** | ![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white) ![GitHub](https://img.shields.io/badge/GitHub-121011?style=flat-square&logo=github&logoColor=white) ![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white) ![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ_IDEA-000000?style=flat-square&logo=intellij-idea&logoColor=white) ![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white) |

---

## 🚀 Projetos em Destaque

### ⚙️ [Task Scheduler BFF](https://github.com/moronioliveira/bff-agendador-tarefas)
Arquitetura **BFF (Backend For Frontend)** estruturada em microsserviços integrando três serviços core: agendamento de tarefas, cadastro de usuários e disparador de notificações.
* **Destaques:** Tratamento global de exceções, autenticação JWT via Spring Security e documentação interativa via Swagger.
* **Tech Stack:** Java, Spring Boot, Spring Security, JWT, PostgreSQL, MongoDB, Swagger.

### 🪙 [CriptoApp](https://github.com/moronioliveira/criptoapp)
Aplicação web para monitoramento e consulta em tempo real de cotações e métricas do mercado de criptomoedas.
* **Destaques:** Integração com APIs externas de dados financeiros, manipulação dinâmica de dados assíncronos no front-end e renderização de interfaces reativas com TypeScript e React.
* **Tech Stack:** React, TypeScript, JavaScript, HTML5, CSS3.
* * 🔗 [Acessar Aplicação Online](https://moronioliveira.github.io/Criptoapp/)

### 📄 [Gerador de Atas de Reunião](https://github.com/moronioliveira/Criador-atas)
Aplicação web interativa para automação do processo de geração e organização de atas corporativas.
* **Destaques:** Redução de até 50% no tempo operacional de criação de documentos.
* **Tech Stack:** HTML5, CSS3, JavaScript.
* 🔗 [Acessar Aplicação Online](https://moronioliveira.github.io/Criador-atas/)

---

## 📈 Estatísticas e Performance

![Streak](https://github-readme-streak-stats.herokuapp.com/?user=moronioliveira&theme=radical)

---

## 📫 Conecte-se comigo

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/moroni-oliveira/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:moronioliveira24@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/moronioliveira)
